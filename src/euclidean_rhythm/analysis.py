...
from .operations import necklace
...
def is_euclidean(rhythm: list[int]) -> bool:
    """Check whether a rhythm is a rotation of the Euclidean rhythm E(k, n).

    A binary rhythm is Euclidean if it equals some rotation of the canonical
    Euclidean rhythm E(k, n) generated by Bjorklund's algorithm, where k is the
    number of onsets and n is the length. Equivalently, the rhythm shares its
    rotation class (necklace) with euclidean(pulses=k, steps=n): a rhythm is
    Euclidean iff necklace(rhythm) == necklace(euclidean(pulses=k, steps=n)).

    Because Euclidean rhythms distribute onsets as evenly as possible, this is a
    test for maximal evenness up to rotation. Many traditional patterns pass:
    the son clave E(3, 8) and all its rotations, the bossa nova E(5, 8), the
    tresillo, the Bossa-Nova E(5, 16), and so on. A clustered pattern such as
    [1, 1, 1, 0, 0, 0, 0, 0] (three onsets bunched together) is not Euclidean.

    The two trivial classes are always Euclidean: the all-rest rhythm (k = 0)
    and the all-onset rhythm (k = n).
    ...
    -------
    bool
        True if the rhythm is a rotation of E(k, n) for its own onset count k
        and length n.

    Raises
    ------
    ValueError
        If the rhythm is empty, or if any value is not 0 or 1.

    Examples
    --------
    >>> is_euclidean([1, 0, 0, 1, 0, 0, 1, 0])  # son clave E(3, 8)
    True
    >>> is_euclidean([0, 1, 0, 0, 1, 0, 1, 0])  # a rotation of the son clave
    True
    ...
    n = len(rhythm)
    k = sum(rhythm)
    reference = euclidean(pulses=k, steps=n)
    return necklace(rhythm) == necklace(reference)
